...
% NOTE: the published rows below are transcribed from memory of the
% LorentzNet paper's comparison table -- VERIFY each against the
% original papers (accuracy, AUC, and 1/eps_B at eps_S = 0.3) before
% submission. Do not trust these values as-is.
...
passing, no attention, three to five orders of magnitude fewer
parameters than the dedicated taggers -- reaches $\mathrm{AUC} =
...
\begin{table}[t]
  ...
  \begin{tabular}{lrccc}
    ...
    ParticleNet~\citep{qu2020particlenet}   & $\sim$498k  & $0.940$ & $0.986$  & $\sim$1615 \\
    LorentzNet~\citep{gong2022lorentznet}   & $\sim$220k  & $0.942$ & $0.987$  & $\sim$2195 \\
    PELICAN~\citep{bogatskiy2022pelican}    & $\sim$208k  & $0.943$ & $0.987$  & $\sim$2250 \\
    \bottomrule
  \end{tabular}
\end{table}
